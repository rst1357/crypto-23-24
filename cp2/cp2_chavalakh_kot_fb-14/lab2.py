...
def plot_key_length_vs_index(text):
    key_lengths = list(range(1, 33))
    index_values = []

    for length in key_lengths:
        index = calculate_index_for_encr(text, length)
        index_values.append(index)

    plt.plot(key_lengths, index_values, marker='.')
...
for kkey in keys:
    print(f'Key: {kkey}')
    index = calculate_index(vigenere_encrypt(cleaned_text, kkey))
    print(f'Index: {index}\n')
    index_dict[len(kkey)] = index
plt.plot(list(index_dict.keys()), list(index_dict.values()), marker = ".")
plt.show()
...
plot_key_length_vs_index(cleaned_text)
