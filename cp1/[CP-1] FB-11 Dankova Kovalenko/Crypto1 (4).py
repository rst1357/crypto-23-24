...
def russian_letter_frequency(text):
   ...
   file = open(r'C:\Users\Admin\PycharmProjects\crypto1\crypto1.txt', encoding ='utf-8')
   text = file.read()
   ...
   all_bigrams2 = [text[i:i + 2] for i in range(0, len(text)-1, 2)]
...
if __name__ == "__main__":
    ...
    #letter_frequencies_without_spaces, all_bigram_frequencies_without_spaces = russian_letter_frequency(input_text_without_spaces)

    h1_with_spaces = calculate_entropy(letter_frequencies_with_spaces)
    h2_with_spaces = calculate_entropy(all_bigram1_frequencies_with_spaces)/2
    h22_with_spaces = calculate_entropy(all_bigram2_frequencies_with_spaces)/2

    #h1_without_spaces = calculate_entropy(letter_frequencies_without_spaces)
    #h2_without_spaces = calculate_entropy(all_bigram_frequencies_without_spaces)

    print("З пробілами:")
    print("Частота всіх літер: ")
    for letter, count in sorted(letter_frequencies_with_spaces.items()):
        print(f"{letter}: {count}")
    print("Частота біграм")
    print("Частота біграм по зміщенню 1")
    for bigram1, count in sorted(all_bigram1_frequencies_with_spaces.items()):
        print(f"{bigram1}: {count}")
    print("Частота біграм по зміщенню 2")
    for bigram2, count in sorted(all_bigram2_frequencies_with_spaces.items()):
    ...
    print(f"H22 (Ентропія для біграм): {h22_with_spaces}")

    #print("\nБез пробілів:")
    #print(f"Частота літер: {letter_frequencies_without_spaces}")
    #print(f"Частота біграм: {all_bigram_frequencies_without_spaces}")
