...
"""
def count_bigram_frequency(text_to_freq):
    bigram_freq_dict = {}
    mass = [text_to_freq[i:i + 2] for i in range(0, len(text_to_freq), 2)]
    mass_unique = list(set(mass))
    for i in mass_unique:
        bigram_freq_dict[i] = round(mass.count(i) / len(mass), 15)
...
if __name__ == '__main__':
    with open('11.txt', 'r', encoding='utf8') as f1_read:
        ciphertext = f1_read.read()
        ciphertext = ciphertext.replace('\n', '')
    bigram_freq_top = count_bigram_frequency(ciphertext)
